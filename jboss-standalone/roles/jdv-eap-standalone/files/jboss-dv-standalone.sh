...
# pidfile: /var/run/jboss-dv/jboss-dv-standalone.pid
# config: /etc/jboss-dv/jboss-dv.conf
...
if [ -z "$JBOSS_CONF" ]; then
  JBOSS_CONF="/etc/jboss-dv/jboss-dv.conf"
fi
...
if [ -z "$JBOSS_HOME" ]; then
  JBOSS_HOME=/usr/share/jboss-dv/jboss-eap-6.4
fi
export JBOSS_HOME

if [ -z "$JBOSS_PIDFILE" ]; then
  JBOSS_PIDFILE=/var/run/jboss-dv/jboss-dv-standalone.pid
fi
export JBOSS_PIDFILE

if [ -z "$JBOSS_CONSOLE_LOG" ]; then
  JBOSS_CONSOLE_LOG=/var/log/jboss-dv/console.log
fi
